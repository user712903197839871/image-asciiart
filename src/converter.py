        ...
        for y in range(0, new_image_height):
            for x in range(0, maximal_width):
                ...
                if loaded_img[x, y] > remainder:
                    value = loaded_img[x, y] - divisor
                else:
                    value = loaded_img[x, y]

                index = round(value / divisor)
...
if __name__ == "__main__":
    ...
    # get the charlist
    values_list = charset_getter.getCharset()

    if character_set_index >= len(values_list) or character_set_index < 0:
        print("[ERROR] character mode invalid")
        exit(1)

    # because we're storing the whole metadata int charset file, we choose 2, 
    # as 2 is the order of character set in that file
    character_set = values_list[character_set_index][2]
